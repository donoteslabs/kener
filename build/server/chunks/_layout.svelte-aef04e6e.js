...
const Nav = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `<div class="one"></div> <header class="relative z-50 w-full"><div class="container flex h-14 items-center"><a${add_attribute("href", data.site.home ? data.site.home : base, 0)} class="mr-6 flex items-center space-x-2">${data.site.logo ? `<img${add_attribute("src", data.site.logo, 0)} class="h-8"${add_attribute("alt", data.site.title, 0)} srcset="">` : ``} ${data.site.title ? `<span class="hidden text-[15px] font-bold md:inline-block lg:text-base">${escape(data.site.title)}</span>` : ``}</a> <div class="blurry-bg flex w-full justify-end">${data.site.nav ? `<nav class="flex-wrap mr-4 items-center space-x-6 text-sm font-medium hidden md:flex">${each(data.site.nav, (navItem) => {
    return `<a${add_attribute("href", navItem.url, 0)}>${escape(navItem.name)} </a>`;
  })}</nav> ${validate_component(Root, "DropdownMenu.Root").$$render($$result, { class: "" }, {}, {
    default: () => {
      return `${validate_component(Trigger, "DropdownMenu.Trigger").$$render($$result, { class: "mr-2 flex md:hidden" }, {}, {
        default: () => {
          return `${validate_component(Button, "Button").$$render($$result, { variant: "outline", size: "sm" }, {}, {
            default: () => {
              return `${validate_component(Menu$1, "Menu").$$render($$result, { size: 14 }, {}, {})}`;
            }
          })}`;
        }
      })} ${validate_component(Dropdown_menu_content, "DropdownMenu.Content").$$render($$result, {}, {}, {
        default: () => {
          return `${each(data.site.nav, (navItem) => {
...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `${data.showNav ? `${validate_component(Nav, "Nav").$$render($$result, { data }, {}, {})}` : ``} ${$$result.head += `<!-- HEAD_svelte-rp3n5o_START -->${$$result.title = `<title>${escape(data.site.title)}</title>`, ""}<link rel="icon" id="kener-app-favicon" href="${escape(base, true) + "/logo96.png"}">${each(Object.entries(data.site.metaTags), ([key, value]) => {
    return `<meta${add_attribute("name", key, 0)}${add_attribute("content", value, 0)}>`;
  })}<!-- HEAD_svelte-rp3n5o_END -->`, ""} ${slots.default ? slots.default({}) : ``} ${data.showNav && !!data.site.footerHTML ? `<footer class="z-10 py-6 md:px-8 md:py-0"><div class="container relative flex max-w-[890px] flex-col items-center justify-center gap-4 pl-0 md:h-24 md:flex-row"><div class="flex flex-col items-center gap-4 px-8 md:flex-row md:gap-2 md:px-0"><p class="text-center text-sm leading-loose text-muted-foreground md:text-left"><!-- HTML_TAG_START -->${data.site.footerHTML}<!-- HTML_TAG_END --></p></div></div></footer>` : ``}`;
});

export { Layout as default };
//# sourceMappingURL=_layout.svelte-aef04e6e.js.map
